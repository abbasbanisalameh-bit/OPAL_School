...
class NavigationConsolidationTests(TestCase):
    ...
    def test_management_sidebar_r110_exact_order(self):
        administrator = User.objects.create_superuser(
            username="nav_r110_root", email="nav_r110_root@example.com", password="pass"
        )
        sections = navigation_sections_for_user(administrator, route_name="dashboard:home", query_params={})
        labels = [section["label"] for section in sections]
        self.assertEqual(
            labels,
            [
                "الرئيسية", "منصة أوبال التعليمية", "الطلبة", "المعلمين",
                "أولياء الأمور", "الصفوف والشعب", "المواد الدراسية", "الامتحانات",
                "الجدول الأسبوعي", "المالية", "المواصلات", "الوثائق",
                "إعدادات النظام", "الملف الشخصي",
            ],
        )
        keys = [item["key"] for section in sections for item in section["items"]]
        self.assertEqual(len(keys), len(set(keys)))
        self.assertTrue(all(len(section["items"]) == 1 for section in sections))

    def test_management_sidebar_r110_single_item_sections_render_as_direct_links(self):
        sidebar = (Path(__file__).resolve().parents[1] / "templates" / "includes" / "sidebar.html").read_text(encoding="utf-8")
        self.assertIn('{% if section.items|length == 1 %}', sidebar)
        self.assertIn('<span>{{ section.label }}</span>', sidebar)
        self.assertIn('class="opal-menu-group" data-sidebar-group', sidebar)
        self.assertIn('data-sidebar-toggle', sidebar)


    def test_r110_system_settings_internal_actions_have_one_entry_and_workflow_order(self):
        template = (Path(__file__).resolve().parents[1] / "templates" / "core" / "system_settings.html").read_text(encoding="utf-8")
        routes = [
            "core:branch_list",
            "core:system_updates",
            "accounts:role_list",
            "enterprise_ops:permission_matrix",
            "core:operations_center",
            "core:integrity_center",
            "core:production_launch_preparation",
            "learning_platform:manager_school_access",
            "openemis:settings",
            "openemis:logs",
            "development_center:dashboard",
        ]
        positions = {route: template.find("{% url '" + route + "'") for route in routes}
        self.assertEqual(template.count("{% url 'core:system_updates' %}"), 1)
        self.assertEqual(template.count("{% url 'core:production_launch_preparation' %}"), 1)
        present = [route for route in routes if positions[route] >= 0]
    ...
    def test_management_sidebar_r110_keeps_logout_outside_navigation_entries(self):
        sidebar = (Path(__file__).resolve().parents[1] / "templates" / "includes" / "sidebar.html").read_text(encoding="utf-8")
        self.assertIn('class="opal-logout-form"', sidebar)
        self.assertIn("تسجيل الخروج", sidebar)
