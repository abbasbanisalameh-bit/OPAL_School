...
OPERATIONS = [
    _op("transport-dashboard", "settings", "المواصلات", "transport:transport-dashboard", "المسارات والسائقون والطلاب والتتبع.", roles=(MANAGEMENT, DRIVER, PARENT), icon="bus-front-fill", keywords="مواصلات حافلة سائق رحلة موقع تتبع", order=25),
    _op("operations-center", "overview", "دليل العمليات الموحد", "core:operations_center", "دليل مختصر لإجراءات النظام ومساراتها.", icon="signpost-split-fill", keywords="دليل خريطة بحث عمليات", order=1),
    _op("executive-dashboard", "overview", "الرئيسية", "dashboard:home", "مؤشرات الإدارة والمتابعة.", icon="grid-1x2-fill", keywords="لوحة مدير مؤشرات رضا", order=2),
    _op("learning-platform", "overview", "منصة أوبال التعليمية", "learning_platform:manager_dashboard", "إدارة المنصة التعليمية من النظام الموحد.", icon="play-btn-fill", keywords="منصة تعلم دورات اشتراكات", order=3),

    _op("student-list", "students", "الطلبة", "students:student_list", "قائمة الطلاب وبطاقة الطالب 360.", icon="people-fill", keywords="طلاب طالب 360 ملف", order=10),
    ...
    _op("families", "students", "أولياء الأمور", "parent_portal:family_management", "إدارة الأسرة والحساب والأبناء وكشف الأسرة من مكان واحد.", icon="person-vcard-fill", keywords="ولي أمر أسرة إخوة حساب", order=14),
    ...
    _op("academic-structure", "academics", "الصفوف والشعب", "academics:academic_structure", "الصفوف والشعب والأعوام والخطة الدراسية.", icon="diagram-3-fill", keywords="صفوف شعب رسوم هيكل", order=20),
    ...
    _op("subjects", "academics", "المواد الدراسية", "academics:subject_list", "تعريف مواد الصف وحصصها الأسبوعية داخل العام الدراسي من مصدر واحد.", icon="book-half", keywords="مادة مواد خطة منهاج حصص أسبوعية", order=23),
    _op("teachers", "academics", "المعلمين", "teachers:dashboard", "ملفات المعلمين والحسابات والتكليفات التدريسية.", icon="person-badge-fill", keywords="معلم تكليف حساب", order=25),
    _op("timetable", "academics", "الجدول الأسبوعي", "timetable:dashboard", "عرض الجدول الرسمي وتوليده من التكليفات مع منع التعارضات.", icon="calendar3", keywords="جدول حصة معلم صف مادة توليد ذكي", order=26),
    ...
    _op("exams", "academics", "الامتحانات", "exams:exam_cycle_center", "فتح الدورة مرة واحدة وتوزيعها آليًا على تكليفات المعلمين.", icon="file-earmark-check-fill", keywords="امتحان دورة اختبار توزيع", order=32),
    ...
    _op("finance-dashboard", "finance", "المالية", "accounting:dashboard", "التحصيل والمتبقي والإغلاق المالي.", icon="speedometer", keywords="رسوم تحصيل متبقي", order=40),
    ...
    _op("documents", "documents", "الوثائق", "documents:document_list", "الوثائق والتقارير وسجل العمليات.", icon="file-earmark-text-fill", keywords="وثيقة شهادة كتاب", order=60),
    ...
    _op("system-settings",  "settings", "إعدادات النظام", "core:system_settings", "إعدادات المدرسة والصلاحيات والتشغيل.", icon="gear-fill", keywords="إعدادات مدرسة شعار", order=70),
...
SIDEBAR_SECTIONS = {
    # The management sidebar exposes exactly one gateway for each requested
    # domain. Detail actions remain inside the owning page/workflow.
    MANAGEMENT: (
        {"label": "الرئيسية", "items": ("executive-dashboard",)},
        {"label": "منصة أوبال التعليمية", "items": ("learning-platform",)},
        {"label": "الطلبة", "items": ("student-list",)},
        {"label": "المعلمين", "items": ("teachers",)},
        {"label": "أولياء الأمور", "items": ("families",)},
        {"label": "الصفوف والشعب", "items": ("academic-structure",)},
        {"label": "المواد الدراسية", "items": ("subjects",)},
        {"label": "الامتحانات", "items": ("exams",)},
        {"label": "الجدول الأسبوعي", "items": ("timetable",)},
        {"label": "المالية", "items": ("finance-dashboard",)},
        {"label": "المواصلات", "items": ("transport-dashboard",)},
        {"label": "الوثائق", "items": ("documents",)},
        {"label": "إعدادات النظام", "items": ("system-settings",)},
        {"label": "الملف الشخصي", "items": ("profile",)},
    ),
    # Role-specific sidebars remain unchanged in scope: R110 preserves and validates
    # the requested management navigation and does not alter existing role contracts.
    TEACHER: (
...
ACTIVE_ROUTE_ALIASES = {
    # All existing system/configuration routes stay under the single settings
    # gateway instead of creating parallel sidebar entries.
    "system-settings": frozenset({
        "core:system_settings",
        "core:branch_list",
        "core:integrity_center",
        "core:system_updates",
        "openemis:settings",
        "openemis:logs",
        "accounts:role_list",
        "enterprise_ops:permission_matrix",
        "development_center:dashboard",
    }),
