...
def audit_canonical_visible_entries(root: Path | None = None) -> list[AuditIssue]:
    root = (root or project_root()).resolve()
    issues: list[AuditIssue] = []
    # Canonical-entry audit follows the current IA: a route may be linked from
    # its owning gateway and from the detail/form page of the same workflow.
    # Do not force daily-management routes back into the Grades/Sections page;
    # that page is intentionally limited to grade/section administration.
    allowed_locations = {
        "admissions:direct_registration": {
        ...
        "academics:subject_list": {"templates/academics/subject_form.html", "dashboard/templates/dashboard/home.html"},
        "timetable:dashboard": {"templates/timetable/form.html", "dashboard/templates/dashboard/home.html"},
        "timetable:schedule_settings": set(),
        "timetable:absence_center": {"templates/timetable/dashboard.html"},
        "exams:exam_list": {
    ...
    # The executive dashboard renders the same canonical navigation catalogue
    # dynamically inside the quick-access gateway. Dynamic hrefs therefore do
    # not appear as literal {% url %} tokens in the template scanner. Treat the
    # dashboard quick-access gateway as a canonical visible location for these
    # two domain gateways.
    dynamic_dashboard_routes = {
        "academics:subject_list",
        "timetable:dashboard",
    }
    dashboard_quick_access = root / "dashboard" / "templates" / "dashboard" / "home.html"
    dashboard_uses_quick_access = False
    if dashboard_quick_access.is_file():
        try:
            dashboard_source = dashboard_quick_access.read_text(encoding="utf-8")
            dashboard_uses_quick_access = (
                "opal-dashboard-quick-grid" in dashboard_source
                and "opal_navigation_sections" in dashboard_source
            )
        except (OSError, UnicodeDecodeError):
            dashboard_uses_quick_access = False
    ...
    for route_name, allowed in allowed_locations.items():
        ...
        if route_name in dynamic_dashboard_routes and dashboard_uses_quick_access:
            actual.add("dashboard/templates/dashboard/home.html")
        if actual != allowed:
            issues.append(
                AuditIssue(
    ...
    expected_kpi_links = (
        ("kpi-fees", "accounting:dashboard", "#fee-summary"),
        ("kpi-students", "students:student_list", "#student-list"),
        ("kpi-attendance", "dashboard:attendance_detail", ""),
        ("kpi-teachers", "teachers:dashboard", ""),
    )
    ...
    # Communication functions keep their owning gateways; the reference dashboard
    # intentionally does not duplicate them as direct doors.
