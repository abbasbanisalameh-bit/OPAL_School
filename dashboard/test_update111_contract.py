...
class Update111NotificationLayerContractTests(SimpleTestCase):
    ...
    def test_topbar_stacking_context_is_above_sticky_module_navigation(self):
        topbar_css = (self.root / "static" / "css" / "opal_theme_system.css").read_text(encoding="utf-8")
        main_css = (self.root / "static" / "css" / "opal_theme_system.css").read_text(encoding="utf-8")
    ...
    def test_notification_ancestors_do_not_clip_the_dropdown(self):
        css = (self.root / "static" / "css" / "opal_theme_system.css").read_text(encoding="utf-8")
        for selector in (
            ".opal-topbar-shell",
            ".opal-topbar-main",
            ".opal-topbar-actions",
