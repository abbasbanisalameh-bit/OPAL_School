...
from core.subject_colors import canonical_subject_color
...
def build_exam_dashboard_context(request):
    ...
    for row in teacher_results:
        ...
        subject_name = row.get("exam__subject__name", "")
        row["opal_subject_color"] = canonical_subject_color(subject_name)
    context = scope_filter_context(scope)
