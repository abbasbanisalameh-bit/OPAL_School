...
def dashboard(request):
    ...
    current_assignments = TeacherAssignment.objects.filter(
        teacher__school=school, is_active=True, academic_year__is_current=True
    ...
    teachers = Teacher.objects.filter(school=school).select_related("school", "branch", "user").prefetch_related(
        ...
        assignments_count=Count("assignments", filter=Q(assignments__is_active=True, assignments__academic_year__is_current=True), distinct=True)
    )
    if query:
        teachers = teachers.filter(Q(full_name__icontains=query) | Q(employee_number__icontains=query) | Q(phone__icontains=query) | Q(specialization__icontains=query))
    if status == "active": teachers = teachers.filter(is_active=True)
    elif status == "inactive": teachers = teachers.filter(is_active=False)
    teacher_rows = list(teachers.order_by("full_name"))
    period = timezone.localdate().replace(day=1)
    snapshots = {row.teacher_id: row for row in TeacherPerformanceSnapshot.objects.filter(teacher_id__in=[t.pk for t in teacher_rows], period=period)}
    for teacher in teacher_rows: teacher.tpi_snapshot = snapshots.get(teacher.pk)
    tpi = management_tpi_snapshot_context(school=school)
    all_teachers = Teacher.objects.filter(school=school)
    context = {
        "teachers_count": all_teachers.count(), "active_count": all_teachers.filter(is_active=True).count(),
        "assignments_count": TeacherAssignment.objects.filter(teacher__school=school, is_active=True, academic_year__is_current=True).count(),
        "unassigned_count": all_teachers.annotate(current_assignment_count=Count("assignments", filter=Q(assignments__is_active=True, assignments__academic_year__is_current=True), distinct=True)).filter(current_assignment_count=0).count(),
        "teachers": teacher_rows, "query": query, "status": status,
        "tpi_period": tpi["period"], "tpi_top_teachers": tpi["top_teachers"],
    }
