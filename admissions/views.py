...
from core.identifiers import normalize_phone
...
def fee_payment_archive(request):
    ...
    if student_id:
        payments = payments.filter(main_student_id=student_id)
    ...
    if student_id:
        legacy_receipts = legacy_receipts.filter(payment__invoice__student_id=student_id)
    if not show_deleted:
