...
/* OPAL 3-theme controller: dark -> green -> light -> dark */
(function () {
  "use strict";
  ...
  function applyTheme(mode) {
    ...
    const currentIndex = modes.indexOf(resolved);
    const next = modes[(currentIndex + 1) % modes.length];

    button.setAttribute(
      "aria-label",
      "التبديل إلى " + labels[next]
    );
    button.setAttribute(
      "title",
      "التبديل إلى " + labels[next]
    ...
    if (icon) {
      icon.className = "bi " + icons[resolved];
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    button.addEventListener("click", function () {
      const current = normalize(
        document.documentElement.getAttribute("data-opal-theme")
      );
...
(function () {
    ...
    function installTooltips() {
        /*
         * Tooltips are opt-in through an explicit title attribute.
         * ARIA labels describe controls for assistive technology and must not
         * automatically become visual tooltips (especially on the mobile
         * sidebar toggle, where they can overlap the brand area).
         */
...
/* ===== OPAL Update 131.7 R91: refined collapsible sidebar groups ===== */
(function () {
    "use strict";
    ...
    function initSidebarGroups() {
        ...
        const storageKey = "opal-sidebar-groups";
        let saved = {};
        try {
        ...
        function setState(group, expanded, persist) {
            const button = group.querySelector("[data-sidebar-toggle]");
            const items = group.querySelector("[data-sidebar-items]");
        ...
        groups.forEach(function (group, index) {
            ...
            const hasActiveItem = !!group.querySelector("a.active, a[aria-current=\"page\"]");
            const stored = typeof saved[index] === "boolean" ? saved[index] : null;
            setState(group, hasActiveItem || stored !== false, false);
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initSidebarGroups, {once: true});
    } else {
        initSidebarGroups();
    }
})();
